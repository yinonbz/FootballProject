...
public class ARankingPolicyTest {
     ...
     */
        ...
        assertEquals(1,infoC.get(0).intValue()); // num of games
        assertEquals(1,infoC.get(1).intValue()); // num of goals F
        assertEquals(2,infoC.get(2).intValue()); //num for goals A
        assertEquals(0,infoC.get(3).intValue()); //num of points
        LinkedList <Integer> infoL = rankTable.get(Liverpool);
        assertEquals(1,infoL.get(0).intValue()); // num of games
        assertEquals(2,infoL.get(1).intValue()); // num of goals F
        assertEquals(1,infoL.get(2).intValue()); //num for goals A
        assertEquals(3,infoL.get(3).intValue()); //num of points
        aRankingPolicy.updateRank(m2, rankTable);
        LinkedList<Integer> infoMU = rankTable.get(ManchesterUnited);
        assertEquals(1,infoMU.get(0).intValue()); // num of games
        assertEquals(0,infoMU.get(1).intValue()); // num of goals F
        assertEquals(0,infoMU.get(2).intValue()); //num for goals A
        assertEquals(1,infoMU.get(3).intValue()); //num of points
        aRankingPolicy.updateRank(m3,rankTable);
        infoMU = rankTable.get(ManchesterUnited);
        assertEquals(2,infoMU.get(0).intValue()); // num of games
        assertEquals(2,infoMU.get(1).intValue()); // num of goals F
